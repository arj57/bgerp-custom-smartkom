...
<div class="center1020">
	...
	<html:form action="/user/empty">
		<input type="hidden" name="forwardFile" value="/WEB-INF/jspf/user/plugin/custom.smartkom/report/old/custom_smo_demontazh_report.jsp"/>
	...
	<%-- <td>Кол-во людей, вышедших на работу</td>--%>
		...
		<c:forEach var="row" items="${result.rowsByIndex}">
			<tr>
				...
				<td>${row[5]}</td>
				<td>${row[6]}</td>
				<td><a href="UNDEF" onclick="$$.process.open( ${row[7]} ); return false;">${row[7]}</a></td>
				<td><a href="UNDEF" onclick="$$.process.open( ${row[8]} ); return false;">${row[8]}</a></td>
				<td>${row[9]}</td>
				<td>${row[10]}</td>
			</tr>
				</c:forEach>
